...
| 桌面布局 | 支持「经典（侧边卡片）」「Deepin Dock」「Windows 任务栏（含开始菜单）」三种风格，可在设置 桌面→桌面布局 切换 |
...
- 新增「Deepin Dock」布局：细顶部状态条 + 全屏应用分页 + 底部居中 Dock。
- 新增「Windows 任务栏」布局：底部全宽任务栏（开始按钮 + 固定应用 + 系统托盘）+ 全屏应用分页，开始按钮打开应用抽屉（当作开始菜单）。
- 桌面布局可在 设置 → 桌面 → 桌面布局 中切换（经典 / Deepin / Windows）。
- 优化应用网格间距，修复应用名被行间裁切的问题。
